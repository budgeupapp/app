...
export default function OtherExpenseStep({
    ...
    updateOtherExpenseFrequency,
    otherExpenseLabel,
    updateOtherExpenseLabel,
    otherExpenseNextDate,
    updateOtherExpenseNextDate,
    terms,
    otherExpenseTermDates,
    updateOtherExpenseTermDates,
    otherExpenseEntryMode,
    updateOtherExpenseEntryMode,
}) {
    const tab = otherExpenseEntryMode || 'yearly'
    ...
    const isMultiDate = otherExpenseFrequency === 'termly'
        ...
        <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
            <div style={{ padding: '18px 24px 0', flexShrink: 0 }}>
                ...
                <p style={{
                    ...
                    Any other regular expense not covered above?
                </p>
            </div>
            ...
            <div style={{
                ...
                <input
                    type="text"
                    placeholder="e.g. Gym, Subscriptions, etc."
                    value={otherExpenseLabel || ''}
                    onChange={(e) => updateOtherExpenseLabel(e.target.value)}
                ...
                <div style={{
                    ...
                    {[
                        ...
                        { id: 'instalment', label: 'Per Payment' },
                    ].map(({ id, label }) => (
                        <button
                            key={id}
                            onClick={() => {
                                updateOtherExpenseEntryMode(id)
                                if (id === 'yearly') {
                                    updateOtherExpenseAmount(rawYearly || '')
                                } else {
                                    updateOtherExpenseAmount(rawPerPayment || '')
                                }
                            }}
                            style={{
                    ...
                    <div ref={freqContainerRef} style={{
                        ...
                        <div style={{
                            ...
                            {FREQ_OPTIONS.map(({ id, label }) => {
                                const selected = otherExpenseFrequency === id
                                return (
                                    <button
                                        key={id}
                                        onClick={() => {
                                            freqTapRef.current = true
                                            const wasMultiDate = otherExpenseFrequency === 'termly'
                                            const goingMultiDate = id === 'termly'
                                            const container = scrollRef.current
                                            const savedScroll = container ? container.scrollTop : 0

                                            updateOtherExpenseFrequency(id)
                        ...
                        <div
                            ref={datesBoxRef}
                            onClick={() => {
                                if (isMultiDate || otherExpenseFrequency === 'yearly') return
                            ...
                            style={{
                                ...
                                padding: '10px 12px',
                                cursor: (isMultiDate || otherExpenseFrequency === 'yearly') ? 'default' : 'pointer',
                            }}
                        >
                            {/* Weekly / Monthly: single next-date accordion */}
                            {!isMultiDate && otherExpenseFrequency !== 'yearly' && (
                                <>
                                    <div style={{
                                        display: 'flex', alignItems: 'center',
                                        justifyContent: 'space-between',
                                    ...
                                    >
                                        <div style={{ marginTop: 10 }}>
                                            <DateRow
                                                label="Next payment"
                                                value={otherExpenseNextDate}
                                                onChange={updateOtherExpenseNextDate}
                                ...
                                <>
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '0 0 6px' }}>
                                        ...
                                        {otherExpenseTermDates && Object.keys(otherExpenseTermDates).length > 0 && (
                                            <button
                                                onClick={(e) => { e.stopPropagation(); updateOtherExpenseTermDates({}) }}
                                                style={{
                                    ...
                                    <div style={{
                                        ...
                                        {(terms || []).map(term => (
                                            <DateRow
                                                key={term.id}
                                                label={term.name}
                                                value={otherExpenseTermDates?.[term.id] || term.start}
                                                onChange={(val) => updateOtherExpenseTermDates({ ...otherExpenseTermDates, [term.id]: val })}
                                                onDateTap={(active) => { dateActiveRef.current = active }}
                                                scrollRef={scrollRef}
                                            />
                                        ))}
