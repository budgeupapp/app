...
export default function OtherExpenseStep({
    ...
    otherExpenseAmountPeriod, updateOtherExpenseAmountPeriod,
    otherExpenseFrequency, updateOtherExpenseFrequency,
    otherExpenseLabel, updateOtherExpenseLabel,
    otherExpenseNextDate, updateOtherExpenseNextDate,
    terms,
    ...
    const handleAmountPeriodChange = (newPeriod) => {
        ...
        if (newPeriod !== 'yearly') {
            updateOtherExpenseFrequency(newPeriod)
        } else {
            if (!otherExpenseFrequency || otherExpenseFrequency === amountPeriod) updateOtherExpenseFrequency('monthly')
        }
    }

    const handleFrequencyChange = (newFreq) => {
        freqTapRef.current = true
        updateOtherExpenseFrequency(newFreq)
    }
            ...
            <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', WebkitOverflowScrolling: 'touch', padding: compact ? '0 24px 8px' : '0 24px 16px', display: 'flex', flexDirection: 'column' }} ref={scrollRef}>
                ...
                {amountPeriod === 'yearly' ? (
                    <div ref={datesBoxRef} style={{ background: 'rgba(224,100,112,0.1)', borderRadius: 10, padding: '10px 12px' }}>
                        ...
                        {freq === 'termly' && (
                            <>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                    {(terms || []).map(term => (
                                        <DateRow key={term.id} label={term.name} value={otherExpenseTermDates?.[term.id] || term.start}
                            ...
                            <>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                    {QUARTER_LABELS.map((label, i) => (
                                        <DateRow key={i} label={label} value={otherExpenseQuarterlyDates?.[i] || QUARTER_DEFAULTS[i]}
                                            onChange={(val) => updateOtherExpenseQuarterlyDates({ ...otherExpenseQuarterlyDates, [i]: val })}
                                            onDateTap={(active) => { dateActiveRef.current = active }} scrollRef={scrollRef} />
                                    ))}
                                </div>
                            </>
                        ...
                        {freq === 'yearly' && (
                            <DateRow label="Payment date" value={otherExpenseNextDate || '2025-09-01'} onChange={updateOtherExpenseNextDate}
                                onDateTap={(active) => { dateActiveRef.current = active }} scrollRef={scrollRef} />
                        )}
                    </div>
                ) : (
