...
export default function UniFeesStep({
    ...
    updateUniFeesAmount,
    uniFeesFrequency,
    updateUniFeesFrequency,
    uniFeesEntryMode,
    updateUniFeesEntryMode,
    uniFeesNextDate,
    updateUniFeesNextDate,
    terms,
    uniFeesTermDates,
    updateUniFeesTermDates,
    compact = false,
}) {
            ...
            {!compact && (
                <div style={{ padding: '18px 24px 0', flexShrink: 0 }}>
                    ...
                    <p style={{
                        ...
                        How much are your tuition fees per year? Most UK students pay £9,250. Only add this if you self-fund your tuition from your bank account.
                    </p>
                </div>
            )}
                    ...
                    <div ref={freqContainerRef} style={{
                        ...
                        <div style={{
                            ...
                            {FREQ_OPTIONS.map(({ id, label }) => {
                                const selected = freq === id
                                return (
                                    <button
                                        key={id}
                                        onClick={() => {
                                            freqTapRef.current = true
                                            updateUniFeesFrequency(id)
                                        }}
                                        style={{
                        ...
                        <div
                            ref={datesBoxRef}
                            onClick={() => {
                                if (isMultiDate || freq === 'yearly') return
                            ...
                            style={{
                                ...
                                padding: '10px 12px',
                                cursor: (isMultiDate || freq === 'yearly') ? 'default' : 'pointer',
                            }}
                        >
                            {/* Monthly: single next-date accordion */}
                            {freq === 'monthly' && (
                                <>
                                    <div style={{
                                        display: 'flex', alignItems: 'center',
                                    ...
                                    >
                                        <div style={{ marginTop: 10 }}>
                                            <DateRow
                                                label="Next payment"
                                                value={uniFeesNextDate || '2025-10-27'}
                                                onChange={updateUniFeesNextDate}
                                ...
                                <>
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '0 0 6px' }}>
                                        ...
                                        {uniFeesTermDates && Object.keys(uniFeesTermDates).length > 0 && (
                                            <button
                                                onClick={(e) => { e.stopPropagation(); updateUniFeesTermDates({}) }}
                                                style={{
                                    ...
                                    <div style={{
                                        ...
                                        {(terms || []).map(term => (
                                            <DateRow
                                                key={term.id}
                                                label={term.name}
                                                value={uniFeesTermDates?.[term.id] || term.start}
