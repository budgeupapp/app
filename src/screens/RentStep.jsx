...
export default function RentStep({
    ...
    rentAmountPeriod, updateRentAmountPeriod,
...
children, }) {
    ...
    const [rawAmount, setRawAmount] = useState(() => { const n = parseFloat(String(rentAmount || '').replace(/,/g, '')); return n ? String(n) : '' })
    ...
    const handleAmountPeriodChange = (newPeriod) => {
        updateRentAmountPeriod(newPeriod)
        if (newPeriod !== 'yearly') {
            updateRentFrequency(newPeriod)
        } else {
            if (!rentFrequency || rentFrequency === amountPeriod) updateRentFrequency('monthly')
        }
    }

    const handleFrequencyChange = (newFreq) => {
        freqTapRef.current = true
        updateRentFrequency(newFreq)
    }
            ...
            <div style={{ overflowY: 'visible', overflowX: 'hidden', WebkitOverflowScrolling: 'touch', padding: compact ? '0 24px 8px' : '0 24px 16px', display: 'flex', flexDirection: 'column' }} ref={scrollRef}>
                ...
                {amountPeriod === 'yearly' ? (
                    <div ref={datesBoxRef} style={{ background: 'rgba(224,100,112,0.1)', borderRadius: 10, padding: '10px 12px' }}>
                            ...
                            <>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '0 0 6px' }}>
                                    ...
                                    {rentTermDates && Object.keys(rentTermDates).length > 0 && (
                                        <button onClick={() => updateRentTermDates({})} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, display: 'flex', alignItems: 'center' }}>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                    {(terms || []).map(term => (
                                        <DateRow key={term.id} label={term.name} value={rentTermDates?.[term.id] || term.start}
                            ...
                            <>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '0 0 6px' }}>
                                    ...
                                    {rentQuarterlyDates && Object.values(rentQuarterlyDates).some((v, i) => v !== QUARTER_DEFAULTS[i]) && (
                                        <button onClick={() => { const defaults = {}; QUARTER_DEFAULTS.forEach((d, i) => { defaults[i] = d }); updateRentQuarterlyDates(defaults) }}
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                    {QUARTER_LABELS.map((label, i) => (
                                        <DateRow key={i} label={label} value={rentQuarterlyDates?.[i] || QUARTER_DEFAULTS[i]}
                        ...
                        {freq === 'yearly' && (
                            <DateRow label="Payment date" value={rentNextDate || '2025-09-01'} onChange={updateRentNextDate}
                                onDateTap={(active) => { dateActiveRef.current = active }} scrollRef={scrollRef} />
                        )}
                    </div>
                ) : (
                    <div ref={datesBoxRef}
                        ...
                        {(freq === 'weekly' || freq === 'monthly') && (
                            <>
                                ...
                                <div onClick={(e) => e.stopPropagation()} style={{ maxHeight: datesExpanded ? 200 : 0, opacity: datesExpanded ? 1 : 0, overflow: 'hidden', transition: 'max-height 0.3s ease, opacity 0.2s ease' }}>
                                    <div style={{ marginTop: 10 }}><DateRow label="Next payment" value={rentNextDate} onChange={updateRentNextDate} onDateTap={(a) => { dateActiveRef.current = a }} scrollRef={scrollRef} /></div>
                                </div>
                            </>
                        )}
                        ...
                        {freq === 'termly' && (
                            <>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '0 0 6px' }}>
                                    ...
                                    {rentTermDates && Object.keys(rentTermDates).length > 0 && (
                                        <button onClick={(e) => { e.stopPropagation(); updateRentTermDates({}) }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, display: 'flex', alignItems: 'center' }}>
                                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#777" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="1 4 1 10 7 10" /><polyline points="23 20 23 14 17 14" /><path d="M20.49 9A9 9 0 0 0 5.64 5.64L1 10m22 4l-4.64 4.36A9 9 0 0 1 3.51 15" /></svg>
                                        </button>
                                    )}
                                </div>
                                <p style={{ fontSize: 10, fontWeight: 500, fontFamily: 'Nunito, sans-serif', color: '#444', margin: '0 0 8px' }}>Defaults to your term start dates — tap to change</p>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                    {(terms || []).map(t => <DateRow key={t.id} label={t.name} value={rentTermDates?.[t.id] || t.start} onChange={(v) => updateRentTermDates({ ...rentTermDates, [t.id]: v })} onDateTap={(a) => { dateActiveRef.current = a }} scrollRef={scrollRef} />)}
                                </div>
                            </>
                        )}
                        ...
                        {freq === 'quarterly' && (
                            <>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '0 0 6px' }}>
                                    <p style={{ fontSize: 14, fontWeight: 600, fontFamily: 'Nunito, sans-serif', color: '#000', margin: 0 }}>Payment dates</p>
                                    {rentQuarterlyDates && Object.values(rentQuarterlyDates).some((v, i) => v !== QUARTER_DEFAULTS[i]) && (
                                        <button onClick={(e) => { e.stopPropagation(); const d = {}; QUARTER_DEFAULTS.forEach((v, i) => { d[i] = v }); updateRentQuarterlyDates(d) }}
                                            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4, display: 'flex', alignItems: 'center' }}>
                                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#777" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="1 4 1 10 7 10" /><polyline points="23 20 23 14 17 14" /><path d="M20.49 9A9 9 0 0 0 5.64 5.64L1 10m22 4l-4.64 4.36A9 9 0 0 1 3.51 15" /></svg>
                                        </button>
                                    )}
                                ...
                                <p style={{ fontSize: 10, fontWeight: 500, fontFamily: 'Nunito, sans-serif', color: '#444', margin: '0 0 8px' }}>When is each quarterly payment due?</p>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                    {QUARTER_LABELS.map((l, i) => <DateRow key={i} label={l} value={rentQuarterlyDates?.[i] || QUARTER_DEFAULTS[i]} onChange={(v) => updateRentQuarterlyDates({ ...rentQuarterlyDates, [i]: v })} onDateTap={(a) => { dateActiveRef.current = a }} scrollRef={scrollRef} />)}
                                </div>
                            </>
                        )}
                    </div>
